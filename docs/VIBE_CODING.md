...
- **Agent Runtime & IA**:
  ...
  - Integration Guide: [`Guia de Integração MiniMax AI no DeskcommCRM`](MINIMAX_INTEGRATION.md) (configuração de credenciais BYOK, Agent Engine e `lib/agent-engine/edge/llm/providers.ts`).
- **Pipeline & CRM Core**:
...
- **Provedores de IA**: Suporte a Anthropic, OpenAI, Google e MiniMax AI (veja o [`Guia de Integração MiniMax AI no DeskcommCRM`](MINIMAX_INTEGRATION.md) para configuração de credenciais BYOK, Agent Engine e allowlist de egress em `providers.ts`).
